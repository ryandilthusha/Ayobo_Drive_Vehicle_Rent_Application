...
namespace Rent_Car
{
    public partial class Give_1D_Hiring_Form : Form
    {
        public Give_1D_Hiring_Form()
        {
            InitializeComponent();

            display_data_grid_view_Vehicle_List();
            display_data_grid_view_OneD_Hire_List();
        }
        ...
        public void display_data_grid_view_OneD_Hire_List()    //For the data grid view
        {
            try
            {
                dt = new DataTable();
                ...
                adpt.Fill(dt);
                dgv_OneD_Hire_List.DataSource = dt;
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        private void dgv_OneD_Hire_List_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            con.Open();
            int ID;

            ID = int.Parse(dgv_OneD_Hire_List.Rows[e.RowIndex].Cells[0].Value.ToString());
            ...
            if (DR1.Read())
            {
                txt_OneD_ID.Text = DR1.GetValue(0).ToString();
                txt_type.Text = DR1.GetValue(1).ToString();
                ...
                txt_Max_hour_limit.Text = DR1.GetValue(9).ToString();
                txt_Extra_hour_charge.Text = DR1.GetValue(10).ToString();
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    con.Open();
                    cmd = new SqlCommand("INSERT INTO Details_OneD_Hire_Table(Veh_Type,Veh_Brand,Veh_Model,Veh_Colour,OneD_Packages,OneD_Per_km_charge,OneD_Max_km_limit,OneD_Extra_km_charge,OneD_Max_hour_limit,OneD_Extra_hour_charge) VALUES('" + txt_type.Text + "' , '" + txt_brand.Text + "' , " +
                        "'" + txt_model.Text + "' , '" + txt_colour.Text + "' , '" + cmb_packages.Text + "' , '" + txt_Per_km_charge.Text + "'  , '" + txt_Max_km_limit.Text + "' , '" + txt_Extra_km_charge.Text + "', '" + txt_Max_hour_limit.Text + "', '" + txt_Extra_hour_charge.Text + "' )", con);
                    cmd.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("One Day Package added successfully!!!");

                    display_data_grid_view_Vehicle_List();
                    display_data_grid_view_OneD_Hire_List();    //data grid view method
            ...
            else
            {
                try
                {
                    ...
                    cmd = new SqlCommand("UPDATE Details_OneD_Hire_Table SET Veh_Type = '" + txt_type.Text + "' ,  Veh_Brand = '" + txt_brand.Text + "', Veh_Model = '" + txt_model.Text + "', " +
                        "Veh_Colour = '" + txt_colour.Text + "', OneD_Packages = '" + cmb_packages.Text + "', OneD_Per_km_charge = '" + txt_Per_km_charge.Text + "', OneD_Max_km_limit = '" + txt_Max_km_limit.Text + "' , OneD_Extra_km_charge = '" + txt_Extra_km_charge.Text + "' , OneD_Max_hour_limit = '" + txt_Max_hour_limit.Text + "' , OneD_Extra_hour_charge = '" + txt_Extra_hour_charge.Text + "'  WHERE OneD_ID = '" + txt_OneD_ID.Text + "' ", con);
                    cmd.ExecuteNonQuery();
                    con.Close();
