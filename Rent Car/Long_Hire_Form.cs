...
namespace Rent_Car
{
    public partial class Long_Hire_Form : Form
    {
        public Long_Hire_Form()
        {
            InitializeComponent();
        ...
        public void clear()
        {
            ...
            txt_total_overnight_charge.Text = "";
            txt_driver_night_charge.Text = "";
        ...
        private void dgv_Hiring_List_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (DR1.Read())
            {
                ...
                Driver_Overnight = Convert.ToInt32(txt_driver_night_charge.Text = DR1.GetValue(9).ToString());      //should Covert to int, becz we calculate them when adding the bill
                Parking_Charge = Convert.ToInt32(txt_parking_charge.Text = DR1.GetValue(10).ToString());            //should Covert to int, becz we calculate them when adding the bill
        ...
        private void btn_Calculate_Click(object sender, EventArgs e)
        {
            ...
            //Nights
            int nights = Convert.ToInt32(txt_days.Text) - 1;

            //display data
            txt_distance.Text = Convert.ToString(distance);
            txt_nights.Text = Convert.ToString(nights);

            if (Convert.ToInt32(txt_distance.Text) <= Convert.ToInt32(txt_Max_km_limit.Text))     //***Distace less than limit
            {
                //FOR DISTANCE
                ...
                txt_total_overnight_charge.Text = Convert.ToString(Convert.ToInt32(txt_driver_night_charge.Text) * Convert.ToInt32(txt_nights.Text));
                txt_total_parking_charge.Text = Convert.ToString(Convert.ToInt32(txt_parking_charge.Text) * Convert.ToInt32(txt_nights.Text));
                txt_total_hire_charge.Text = Convert.ToString(Convert.ToInt32(txt_total_km_charge.Text) + Convert.ToInt32(txt_total_Extra_km_charge.Text)+ Convert.ToInt32(txt_total_overnight_charge.Text) + Convert.ToInt32(txt_total_parking_charge.Text));

            }

            else if (Convert.ToInt32(txt_distance.Text) > Convert.ToInt32(txt_Max_km_limit.Text))     //***Distace over the limit
            {
                //FOR DISTANCE
        ...
        {
            if (txt_type.Text == "" && txt_brand.Text == "" && txt_model.Text == "" && txt_colour.Text == "" && txt_package.Text == "" && txt_Per_km_charge.Text == "" && txt_Max_km_limit.Text == "" && txt_Extra_km_charge.Text == "" && txt_driver_night_charge.Text == "" && txt_parking_charge.Text == "")
            {
            ...
            else
            {
                ...
                newRow.Cells[4].Value = txt_driver_night_charge.Text;
                newRow.Cells[5].Value = txt_parking_charge.Text;
        ...
        private void insert_bill()  //Create a new method to insert records into Bill table
        {
            ...
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("INSERT INTO Bill_LongD_Hire_Table(Driver_Name,Date,Bill_Amount) VALUES('" + lbl_driver.Text + "' , '" + DateTime.Today.Date + "' ," +
                    " '" + finalTotal + "' )", con);
                cmd.ExecuteNonQuery();
                con.Close();
        ...
        public void display_data_grid_view_BILL()    //For the Bill Table data grid view
        {
            try
            {
                ...
                dt = new DataTable();
                con.Open();
                adpt = new SqlDataAdapter("SELECT * FROM Bill_LongD_Hire_Table WHERE Driver_Name = '" + lbl_driver.Text + "'", con);
                adpt.Fill(dt);
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawString("ID     Per_km_charge             Max_km_limit         Extra_km_charge         Driver_night_Charge       Parking_charge        TOTAL ", new Font("Century Gothic", 4, FontStyle.Bold), Brushes.Red, new Point(26, 40));
            ...
            foreach (DataGridViewRow row in dgv_Current_Bill.Rows)
            {
                ...
                Driver_overnight_Charge = Convert.ToInt32(row.Cells["Column5"].Value);
                Parking_charge = Convert.ToInt32(row.Cells["Column6"].Value);
                ...
                e.Graphics.DrawString("" + Driver_overnight_Charge, new Font("Century Gothic", 4, FontStyle.Bold), Brushes.Blue, new Point(235, pos));
                e.Graphics.DrawString("" + Parking_charge, new Font("Century Gothic", 4, FontStyle.Bold), Brushes.Blue, new Point(295, pos));
        ...
        private void btn_refresh_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = con.CreateCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT * FROM Details_LongD_Hire_Table WHERE LongD_Packages ='" + cmb_refresh_category.Text + "'";
            cmd.ExecuteNonQuery();
            con.Close();
